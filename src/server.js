...
app.post("/api/data", upload.single("image"), (req, res) => {
  const { foundBy, itemName, kind, lat, long } = req.body;
  const imageFile = req.file;
  let imageUrl;

  if (!foundBy || !itemName || !kind || !lat || !long || !imageFile) {
  ...
  const insertQuery =
    "INSERT INTO streetart (foundBy, itemName, kind, lat, long, imageUrl) VALUES ($1, $2, $3, $4, $5, $6)";
  db.query(insertQuery, [foundBy, itemName, kind, lat, long, filename], (err, result) => {
    if (err) {
